						...
						<div class="account-order">
							<div class="gray-box">
								...
								<div class="js-list-inner">
									<div class="box-inner order-cart order-list-cart clear" v-for="item in orderinfo">
										<div class="gray-sub-title cart-title">
											<span class="date">2017-06-19</span>
											<span class="order-id"> 订单号： <a href="javascript:;">{{item.orderId}}</a> </span>
											<span class="order-detail">
												<router-link :to="{name:'pay',query:{orderid:item.orderId}}">查看详情&gt;</router-link>
											</span> 
										...
										<div class="cart">
											<div class="cart-items clear" v-for="shop in item.shopdata">
												<div class="prod-info clear">
													<div class="items-thumb">
														<a href="javascript:;" target="_blank"><img :src="shop.ali_image"></a>
													</div>
													<div class="items-params clear">
														<div class="name vh-center">
															<a href="javascript:;" target="_blank" :title="shop.title">{{shop.title}}</a>
														</div>
														<div class="detail"></div>
													</div>
													<div class="num">{{shop.count}}</div>
													<div class="price">¥ {{shop.price}}.00</div>
												</div>
											</div>
										</div>
										<div class="prod-operation">
										<div class="total">¥ {{item.price+item.fremoney}}</div>
										<div class="status">
											<router-link  :to="{name:'pay',query:{orderid:item.orderId}}" class="blue-small-btn js-payment-order" v-if="!item.ispay">现在付款</router-link>
