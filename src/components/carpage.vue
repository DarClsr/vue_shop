			...
			<div class="store-content">
				<div class="cart-box">
					...
					<div class="cart-inner">
						<div class="empty-label" v-if="getshopcount<=0">
							<h3>您的购物车中还没有商品</h3>
							<router-link class="link" to='/allshop'>现在选购</router-link>
						</div>
						<div v-else>
							...
							<div class="cart-table">
								<div class="cart-group">
									<!--购物列表-->
									<div class="cart-top-items" v-for="item in getshopdata">
										<div class="cart-items">
											<div class="items-choose">
												<span class="blue-checkbox-new" :class="{'checkbox-on':item.check}" @click="changecheck(item.sku_id)"><a></a></span>
											</div>
											...
											<div class="name hide-row" >
												<div class="name-table">
													...
													<ul class="attribute">
														<li>{{item.spec_json?item.spec_json.show_name:''}}</li>
													</ul>
												</div>
											</div>
											<div class="operation">
												<a class="items-delete-btn" @click="delshopdata(item.sku_id)" ></a>
											</div>
											
											<div class="subtotal">{{item.price*item.count}}</div>
											<div class="item-cols-num">
												<div class="select js-select-quantity">
													<span class="down" @click="subshopdata(item.sku_id)" :class="{'down-disabled':item.count<=1}">-</span>
													<span class="num">
													...
													<span class="up" @click="plusshopdata(item.sku_id)" :class="{'up-disabled':item.count>=15}">+</span>
					...
					<div class="cart-bottom-bg fix-bottom" v-if="getshopcount>0">
						<div class="cart-bar-operation">
							<div>
								<div class="choose-all js-choose-all">
									<span class="blue-checkbox-new" :class="{'checkbox-on':getallcheck}" @click="changeallcheck(getallcheck)"><a></a></span>
									全选
								</div>
								<div class="delete-choose-goods" @click="delcheckshop()">删除选中的商品</div>
							</div>
						</div>
						<div class="shipping">
							<div class="shipping-box">
								<div class="shipping-total shipping-num">
									<h4 class="">
										已选择 <i>{{checkallcount}}</i> 件商品
									</h4>
									<h5>
										共计 <i >{{getshopcount}}</i> 件商品
									</h5>
								</div>
								<div class="shipping-total shipping-price">
									<h4 class="">
										应付总额：<span>￥</span><i >{{checkallprice}}</i> 
									</h4>
							...
							<router-link class="big-main-btn js-checkout jianguo-blue-main-btn" :class="{'disabled-btn':checkallcount<1}" to='/checkout'><a>现在结算</a></router-link>
						</div>
					</div>
				</div>
			</div>
...
<script>
	export default {
		...
		methods:{
			plusshopdata(id){
				this.$store.commit('pluscardata',id);
			},
			subshopdata(id){
				this.$store.commit('subcardata',id);
			},
			delshopdata(id){
			...
			changeallcheck(allcheck){
				this.$store.commit('changeallcheck',allcheck);
			},
			delcheckshop(){
				this.$store.commit('delcheckshop');
			}
		}
	}
</script>
...
.hander-car .cart-items .item-cols-num, .hander-car .cart-items .operation, .hander-car .cart-items .price, .hander-car .cart-items .subtotal{
  overflow: hidden;
    float: right;
    width: 137px;
    text-align: center;
