	    	...
	    	<div class="content page-order-payment">
	    	    <div class="gray-box clear">
	    	        ...
	    	        <div class="box-inner payment-checkout-panel clear">
	    	            ...
	    	            <span class="prices"> 应付金额：   <em><span>¥ </span>{{orderinfo.price+orderinfo.fremoney}}</em>   </span>
	    	        </div>
	    	    </div>
	    	    ...
	    	    <div class="confirm-detail">
	    	        ...
	    	        <p class="info-detail">姓名：{{orderinfo.user.name}}</p>
	    	        <p class="info-detail">联系电话：{{orderinfo.user.phone}}</p>  <p class="info-detail">详细地址：{{orderinfo.user.province}}{{orderinfo.user.city}}{{orderinfo.user.county}}</p>
	    	    </div>
	    	    <div class="confirm-detail">
	    	        <div class="info-title">发票信息</div>
	    	        <p class="info-detail">发票类型：电子发票</p>
	    	        <p class="info-detail">发票抬头：{{orderinfo.checkon}}</p>
	    	    ...
	    	    <div class="confirm-goods-table">
	    	        <div class="cart-items clear" v-for="item in orderinfo.shopdata">
	    	            <div class="name fn-left hide-row">
	    	                <div class="name-cell"> 
	    	                    <a href="http://www.smartisan.com/shop/#/item/100029901" :title="item.title" target="_blank">{{item.title}}</a><br>
	    	                </div>
	    	            </div>
	    	    ...
	    	    <div class="order-discount-line">
	    	        ...
	    	        <p> 运费： <span>+ ¥ {{orderinfo.fremoney}}</span> </p>
	    	    </div>
	    	</div>
	    </div>
</template>	
	...
	export default {
		...
		methods:{
			paynow(id){
				...
				alert(this.orderinfo.ispay?'支付成功':'支付失败');
