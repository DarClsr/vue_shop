				...
				<div class="item-box">
					<div class="gallery-wrapper">
						<div class="gallery">
							<div class="thumbnail">
								<ul>
									<li :class="{'on':index==imgindex}" @click="tabimg(index)" v-for="(item,index) in itemInfo.ali_images"><img :src="item+'?x-oss-process=image/resize,w_54/quality,Q_90/format,webp'"></li>
								</ul>
							</div>
							<div class="thumb">
								<ul>
									<li :class="{'on':index==imgindex}" v-for="(item,index) in itemInfo.ali_images"><img :src="item+'?x-oss-process=image/resize,w_440/quality,Q_90/format,webp'"></li>
								</ul>
							</div>
						</div>
					</div>
					<div class="banner">
						...
						<div class="sku-dynamic-params-panel">
							<div class="sku-dynamic-params clear">
								...
								<ul class="params-colors">
									<li :class="{'cur':index==colorindex}" v-for="(item,index) in itemInfo.sku_list" @click="tabcolor(index)">
										<router-link :to='"/product/"+item.id'><img :src="'http://img01.smartisanos.cn/'+item.image+'20X20.jpg'"></router-link>
									</li>
								</ul>
							</div>
							<div class="sku-dynamic-params clear">
								...
								<div class="params-detail clear">
									<div class="item-num js-select-quantity">
										<span class="down" :class="count<=1?'':'up-disabled'" @click="reduceshop()">-</span>
										<span class="num">{{count}}</span>
										<span class="up"  :class="count>=15?'up-disabled':''" @click="addshop()">+</span>
									</div>
								</div>
							</div>
						</div>
						<div class="sku-status">
							<div class="cart-operation-wrapper clearfix">
								<span class="blue-title-btn js-add-cart"><a @click="addbuycar">加入购物车</a></span>
								<router-link class="gray-title-btn" to='/buy'><a>现在购买</a></router-link>
							</div>
						</div>
					</div>
				</div>
	...
	export default {
		...
		methods:{
			...
			addbuycar(){
				...
				this.$store.commit('addbuydata',itemdata);
		...
		computed:{
			itemInfo(){
				let itemInfo=null;
				console.log(this.itemdata[0].sku_id==this.itemId)
				console.log(this.itemdata[0].sku_id)
				console.log(this.itemId)
				for(var i=0;i<this.itemdata.length;i++){
					
					if(this.itemdata[i].sku_id== this.itemId){
						itemInfo=this.itemdata[i];
