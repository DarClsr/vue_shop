	  	...
	  	<div id="main">
			<div class="sku-box store-content ">
				<div class="sort-option">
					<ul class="line clear" ref='listlink'>
						<li><a href="javascript:;" class="active" @click="tablink($event)">综合排序</a></li>
						<li><a href="javascript:;"  @click="tablink($event)">销量排序</a></li>
						<li><a href="javascript:;"  @click="reserver();tablink($event)">价格高到低</a></li>
						<li><a href="javascript:;"  @click="sortprice();tablink($event)">价格低到高</a></li>
					</ul>
				</div>
				<div class="gray-box">
					<header class="title-header">
						<h5 class="title">“足迹系列”手感膜</h5>
					</header>
					<div class="item-box">
						<div class="item" v-for="(item,a) in goodlist" :key='a+Math.random()'>
							<moItem :item="item"></moItem>
							</div>
						</div>
					</div>
				</div>
				<maxinfo v-if="showinfo"></maxinfo>
			</div>
		</div>
</template>
<script>
	import modata from '../lib/modata.js';
	import moItem from './moItem.vue';
	import maxinfo from './maxinfo.vue';
	export default {
		data() {
			return {
				goodlist:modata,
		...
		methods:{
			/* 从高到低 */
			reserver(){
			this.$store.commit('reserver',this.goodlist);
			},	
			/* 从低到高 */
			sortprice(){
			this.$store.commit('sortprice',this.goodlist);
			},
			/* 改变选项按钮的方法 */
			tablink(e){
				var list=this.$refs.listlink;
				var a=this.$refs.listlink.querySelectorAll('a');
