<template>
		...
		<div class="sku-box store-content ">
			...
			<div class="gray-box">
				...
				<div class="item-box">
					<div class="item" v-for="(item,a) in allshopdata" :key='a+Math.random()'>
						<allItem :item="item"></allItem>
						</div>
					</div>
				</div>
			</div>
...
<script>
	...
	import allshopdata from '../lib/allshop.js';
	export default {
		data() {
			return {
		...
		methods:{
			/* 从高到低 */
			reserver(){
			this.$store.commit('reserver',this.allshopdata);
			},	
			/* 从低到高 */
			sortprice(){
			this.$store.commit('sortprice',this.allshopdata);
			},
			/* 改变选项按钮的方法 */
			tablink(e){
