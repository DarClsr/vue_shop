<template>
	<div class="account-content">
						<div class="account-address">
							<div class="gray-box clear">
								<div class="title pre-title">
									...
									<span class="gray-blue-btn js-add-address"><a class="add" @click="showbox()">添加新地址</a></span>
								</div>
								<div class="box-inner clear">
									<div class="address-list-item default-item" data-id="765532" v-for="(item,index) in userdata">
										<div class="name fn-left">
											<div class="name-cell">{{item.name}}</div>
										</div>
										<div class="detail fn-left">
											<div class="detail-cell">{{item.province}} {{item.city}} {{item.county}} {{item.add}}</div>
										</div>
										<div class="operation fn-right">
											<div class="operation-cell">
												<a class="gray-edit-btn js-edit-address" @click="edituser(item,index)">修改</a>
												<span class="red-del-btn js-del-address" @click='deluser(index)'> <a>删除</a><em>删除</em> </span>
											</div>
										</div>
										<div class="default fn-right">  
										 <span v-if="item.default" >（默认地址）</span>
										<a v-else @click="checkDefaultHandle(item)">设为默认</a> 
										 </div>
						...
						<address_pop @close="close" v-if="boxinfo" :olduser="olduser" :userindex="userindex"></address_pop>
					</div>
					
</template>

<script>
	import address_pop from './address_pop.vue';
	export default {
		data() {
			return {
