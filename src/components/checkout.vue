						...
						<div class="box-inner js-checkout-address-panel ">
							<div class="address-common-table js-multiple-address-panel">
								<ul class="address-item-list clear js-address-item-list">
									<li class="js-choose-address" v-for="(item,index) in userdata" :class="{'selected-address-item':selectindex==index}" @click="tabchecked(index)">
										<div class="address-item">
											<div class="name-section">  {{item.name}}  </div>
											<div class="mobile-section">{{item.phone}}</div>
											<div class="detail-section"> {{item.province}} {{item.市辖区}} {{item.county}}<br> {{item.add}} </div>
										</div>
									...
									<li class="add-address-item js-add-address" @click="openbox">
										<p>使用新地址</p>
									</li>
								</ul>
							</div>
				...
				<div class="gray-box">
					...
					<div class="box-inner invoice-box js-invoice-box"> 
						...
						<div class="invoice-detail"> 发票抬头： 
							<div class="radio-box"> 
								<label v-for="(a,index) in checkon"> 
									<input type="radio" class="hide"> 
									<span class="blue-radio " :class="{'blue-radio-on':a.static}" @click="tabradio(index)" ><a></a></span>  {{a.name}}
								</label>  
							</div> 
							<div class="module-form-row form-item fn-hide js-invoice-title" v-if="radiocheck"> 
								<div class="module-form-item-wrapper no-icon small-item"> 
									<input type="text" class="js-verify" placeholder="请填写公司发票抬头"> 
								</div> 
							</div> 
						</div> 
				...
				<div class="gray-box">
					...
					<div class="box-inner ui-goods-cart">
						...
						<div class="cart-table">
							<div class="cart-group js-cart-group">
								<div class="cart-items" v-for="item in getshopdata">
					...
					<div class="box-inner"> 
						<div class="order-discount-line"> 
							<p> 商品总计： <span>¥ {{checkallprice}}</span> </p> 
							<p> 运费： <span>+ ¥ {{fremoney}}</span> </p>  
							<p class="discount-line js-discount-cash"> <em>现金券</em>： <span> - 0 </span> </p>  
						</div> 
					</div>
					<div class="box-inner"> 
						<div class="last-payment clear"> 
							<span class="jianguo-blue-main-btn big-main-btn payment-blue-bt fn-right js-checkout" @click='submitorder'> <a>提交订单</a> </span> <span class="prices fn-right">应付金额： <em>¥ {{checkallprice}}</em></span> 
						</div> 
					</div>
				</div>
			</div>
			<!-- v-bind 指令用于响应地更新 HTML 可以用来传输数据 形式如：v-bind:href    缩写为    :href;;
			v-on 指令用于监听DOM事件 可以用来传输事件 形式如：v-on:click  缩写为 @click; -->
			<adduser v-if="showadd" @close='closebox'></adduser>
		</div>
...
<script>
	...
	export default {
		...
		computed:{
			/* 获取结算商品的数量 */
			getshopdata(){
			return this.$store.getters.checkoutshop
			},
			checkallprice(){
		...
		methods:{
			...
			tabradio(index){
				...
				for(var i=0;i<this.checkon.length;i++){
					this.checkon[i].static=false;
				}
				this.checkon[index].static=true;
				if(index==0){
			...
			submitorder(){
				for(var i=0;i<this.checkon.length;i++){
					if(this.checkon[i].static){
						var ondata=this.checkon[i].name;
				...
				var data={
					...
					price:this.checkallprice,
					count:this.checkallcount,
					user:this.selectuser,
					fremoney:this.fremoney,
					checkon:ondata,
					date:idate.getFullFear,
					ispay:false,
				}
				this.$store.commit('submitorder',data);
				this.$router.push({name:'pay',query:{orderid:data.orderId}});
